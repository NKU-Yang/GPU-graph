    ...
    for (int testIndex = 0; testIndex < testTimes; testIndex++) {
        ...
        while (activeNodesNum) {
            ...
            uint staticNodeNum = thrust::reduce(graph.actStaticLablingThrust,
                                                ...
                                                thrust::plus<uint>());
            //cout<<"staticNodeNum after reduce："<<staticNodeNum<<endl;
            ...
            cudaDeviceSynchronize();
            bool*temp=new bool[graph.vertexArrSize];
            cudaMemcpy(temp,graph.isActiveD,sizeof(bool)*graph.vertexArrSize,cudaMemcpyDeviceToHost);
            gpuErrorcheck(cudaPeekAtLastError());
            
            gpuErrorcheck(cudaPeekAtLastError());
            //cout<<"overloadNodeNum after bfs_kernel: "<<overloadNodeNum<<endl;
